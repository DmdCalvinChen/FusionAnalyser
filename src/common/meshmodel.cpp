...
void MeshDocument::setCurrentMeshNotNotifyLayerChanging(int i)
{
	if (i < 0)
	{
		currentMesh = 0;
...
MeshModel * MeshDocument::addNewMesh(QString fullPath, QString label, bool bPushBack, bool setAsCurrent,
	ToothModelType upperOrLowerToothModelMark)
{
    ...
    if(!fullPath.isEmpty())
    {
		...
		newFullPath = fi.absoluteFilePath();
    }

    MeshModel *newMesh = new MeshModel(this,newFullPath/*qPrintable(newFullPath)*/,newlabel, upperOrLowerToothModelMark);
	newMesh->_labelSegmentMark = label;
	newMesh->_fullPathFileNameSegment = fullPath;
	if (bPushBack)
	{
	...
	{
		meshList.insert((--meshList.end()), newMesh);
	}

	if (setAsCurrent && upperOrLowerToothModelMark == UpperToothModel)
	{
...
MeshModel * MeshDocument::addNewMesh(MeshModel *newMesh, QString label, bool setAsCurrent)
{
	QString newlabel = NameDisambiguator(this->meshList, label);
...
MeshModel * MeshDocument::addNewMesh(QString fullPath, QString label, bool setAsCurrent, int iPos)
{
	QString newlabel = NameDisambiguator(this->meshList, label);
	...
	MeshModel *newMesh = new MeshModel(this, fullPath/*qPrintable(fullPath)*/, newlabel);
	meshList.insert(meshList.begin() + iPos, newMesh);
...
MeshModel::MeshModel(MeshDocument *_parent, QString fullFileName, QString labelName, ToothModelType upperOrLowerToothModelMark)
{
    /*glw.m = &(cm);*/
    Clear();
    parent=_parent;
	...
	this->upperOrLowerToothModelMark = upperOrLowerToothModelMark;
}
...
MeshModel::MeshModel(CMeshO *mesh, MeshDocument *Parent, MeshModel *cp)
{
	if (mesh == NULL)
		return;
	parent = Parent;
...
void MeshModel::updateVertexInfoBuffer(MLSceneGLSharedDataContext* glContext)
{
	if (glContext != NULL)
	{
		...
		for (int i = 0; i < this->parent->meshList.size(); ++i)
		{
			glContext->meshAttributesUpdated(this->parent->meshList[i]->id(), false, atts);
			glContext->manageBuffers(this->parent->meshList[i]->id());
		}
...
//void MeshModel::constructOctree()
//{
//    if (octree_ != nullptr)
//    {
//        return;
...
//    int index = 0;
//    for (auto& vert : cm.vert)
//    {
//        nodes.push_back(CloudNode(index, vert.P(), vert.N()));
//        ++index;
//    }
//
//    vcg::tri::UpdateBounding<CMeshO>::Box(cm);
//    vcg::Box3f box = cm.bbox;
//
//    octree_ = new CloudOctree(nodes, box, nullptr);
