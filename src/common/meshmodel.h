...
enum ToothModelType
{
	...
	UpperSingleRoot,
	LowerSingleRoot,
	UpperSingleAccessory,
	LowerSingleAccessory,
	UpperSingleComparedTooth,
	LowerSingleComparedTooth,
	UpperSingleComparedRoot,
	LowerSingleComparedRoot,
	UpperSingleComparedGum,
	LowerSingleComparedGum,
	UpperSingleBasalLabel,
	LowerSingleBasalLabel,
	UpperSingleExportableMesh,
	LowerSingleExportableMesh,
	UpperJawBoneMesh,
	LowerJawBoneMesh,
	UpperGingivaMesh,
	LowerGingivaMesh,
	UpperCTRootModel, // 上下颌CT根模型标志
	LowerCTRootModel,
	UpperSourceMesh,
	LowerSourceMesh
};
...
public:
	...
	*/
	...
	enum AsyncState {
		ASYNC_NONE = 0x00000000,
		ASYNC_UPDATING_GUM = 0x00000001,
		ASYNC_UPDATING_ANIMATION = 0x00000002,
		ASYNC_UPDATING_RESULT = 0x00000004,
		ASYNC_EXPORTING_RESULT = 0x00000008,
	};

	MeshModel(MeshDocument* parent, QString fullFileName, QString labelName, ToothModelType upperOrLowerToothModelMark = UpperToothModel);
	MeshModel(MeshModel* cp);
	MeshModel(CMeshO* mesh, MeshDocument* Parent, MeshModel* cp);
	~MeshModel();

	MeshDocument* parent;

	CMeshO cm;
	ToothModelType upperOrLowerToothModelMark = UpperToothModel;
	PlaneView planeView;
	void updateVertexInfoBuffer(MLSceneGLSharedDataContext* glContext);
	//AxisEachToothSegmented axis_local_;

	//CloudOctree *octree_ = nullptr;
...
private:
	...
	int _id;
	bool modified;
	CullFaceMode cull_mode_ = CULL_BACK;

public:

	QString _labelSegmentMark;
	QString _fullPathFileNameSegment;
	...
	CullFaceMode cullMode() { return cull_mode_; }
	void setCullMode(CullFaceMode _mode) { cull_mode_ = _mode; }
...
public:
	...
	void setCurrentMeshNotNotifyLayerChanging(int i);
...
private:
	...
	*/
	...
	//QString segmentProjectFileName;
	//QString alignProjectFileName;
...
public:
	...
	QString getDocLabel() const { return documentLabel; }

	//void setSegmentProjectLabel(const QString& segmentProLb)
	//{
	//	segmentProjectFileName = segmentProLb;
	//}

	//QString getSegmentProjectLabel() const {
	//	return segmentProjectFileName;
	//}

	//void setAlignProjectLabel(const QString& alignProLb) {
	//	alignProjectFileName = alignProLb;
	//}

	//QString getalignProjectLabel() {
	//	return alignProjectFileName;
	//}
...
public:
	...
	MeshModel* addNewMesh(QString fullPath, QString Label, bool bPushBack = true, bool setAsCurrent = true, ToothModelType upperOrLowerToothModelMark = UpperToothModel);
	MeshModel* addNewMesh(MeshModel* newMesh, QString label, bool setAsCurrent);
	MeshModel* addNewMesh(QString fullPath, QString label, bool setAsCurrent, int iPos);
