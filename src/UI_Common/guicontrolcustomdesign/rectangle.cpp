...
namespace ui_common
{
Rectangle::Rectangle(const QString& _desrction, const QString& _helpDesrction,
...
void Rectangle::setAttributeInfomation(const BaseAttributeInfoButton& _baseAttributeInfomation)
{
    this->baseAttributeInfomation = _baseAttributeInfomation;
}

void Rectangle::setAttributeInfomationBtnWidthScalse(float _scale)
{
    this->baseAttributeInfomation.setBaseAttributeWidthBorderScaleRadius(_scale);
}

void Rectangle::mousePressEvent(QPoint posCursor)
{
...
void Rectangle::draw(QWidget* _parent, QPainter* painter, QPoint posCursor,
                     const Matrix44m& _matrixTransform, bool& _bIsPickedThisControl)
{
    ...
    auto height = baseAttributeInfomation.getBaseAttributeHeight();
    float widthBorder = length * baseAttributeInfomation.getBaseAttributeWidthBorderScaleRadius();
    ...
    glBegin(GL_QUADS);
    auto v = matrixTransform * Point3m(-length - widthBorder, height + widthBorder, 0.0f);
    glVertex2f(v.X(), v.Y());
    v = matrixTransform * Point3m(-length - widthBorder, -height - widthBorder, 0.0f);
    glVertex2f(v.X(), v.Y());
    v = matrixTransform * Point3m(length + widthBorder, -height - widthBorder, 0.0f);
    glVertex2f(v.X(), v.Y());
    v = matrixTransform * Point3m(length + widthBorder, height + widthBorder, 0.0f);
    ...
    glBegin(GL_QUADS);
    v = matrixTransform * Point3m(-length, height, 0.0f);
    glVertex2f(v.X(), v.Y());
    v = matrixTransform * Point3m(-length, -height, 0.0f);
    glVertex2f(v.X(), v.Y());
    v = matrixTransform * Point3m(length, -height, 0.0f);
...
void Rectangle::mouseMoveCustomEvent(QWidget* parent, QPainter* painter, QPoint posCursor)
{
...
bool Rectangle::cursorIsStayOnThisButton(QPoint cursorPos, Point3m& transformCursor)
{
...
void Rectangle::setCurrentPlayStatus(bool _bIsPlaying /* = true */)
{
    this->baseAttributeInfomation.setBaseAttributePlayStatus(_bIsPlaying);
}

void Rectangle::setVisible(bool _bIsVisible)
{
    baseAttributeInfomation.setBaseAttributeIsVisible(_bIsVisible);
}

void Rectangle::setChecked(bool _bIsChecked)
{
    // todo something when the button is pressed.
...
void Rectangle::setBaseAttributeInfoButtonColor(const Point4f& _color)
{
    this->baseAttributeInfomation.setBaseAttributeForegroundColor(_color);
}

void Rectangle::setBaseAttributeInfoButtonBackgroundColor(const Point4f& _color)
{
