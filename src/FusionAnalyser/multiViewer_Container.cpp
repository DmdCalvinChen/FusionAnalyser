...
void MultiViewer_Container::closeEvent( QCloseEvent *event )
{
	if (meshDoc.hasBeenModified())
	{
		...
		//	this,  tr("MeshLab"), tr("Project '%1' modified.\n\nClose without saving?").arg(meshDoc.getDocLabel()),
		//	QMessageBox::Yes|QMessageBox::No,
		//	QMessageBox::No);

		if(!UiUtilityTools::getInstance()->showQuestionMessageBox(tr("Info"), tr("Project '%1' modified.\n\nClose without saving?").arg(meshDoc.getDocLabel())) > 0)	// don't close please!
		{
			event->ignore();
			return;
		}
